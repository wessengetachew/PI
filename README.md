...
<div class="presets-wrap">
  ...
  <div class="pg-label gold">Gap Waves <span style="font-size:.5rem;opacity:.5">— run decomposition below to refine</span></div>
...
<div class="sphere-wrap">
  ...
  <div class="rot-controls">
    <span class="rot-hint">⟳ drag sphere to rotate</span>
    <button class="ctrl" id="btnAutoRot">⟳ Auto-Rotate</button>
    <button class="ctrl" id="btnResetRot" style="font-size:.63rem;padding:6px 11px">↺ Reset View</button>
  </div>

  <div class="formula-box">
    f(θ) = <em id="fA">1.9</em> + sin(<em id="fB">3</em>θ) · sin(<em id="fC">3</em>θ / <em id="fD">32</em>)
  </div>
  <canvas id="cartCanvas"></canvas>

  <!-- Multi-gap selector — main sphere area, shown after compute -->
  <div id="mainGapSelector" style="display:none;width:100%;max-width:540px;margin-bottom:4px;">
    <div style="font-size:.53rem;text-transform:uppercase;letter-spacing:.16em;color:rgba(255,215,0,.55);margin-bottom:6px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
      <span>🎨 Highlight gap classes on sphere &amp; cart</span>
      <span style="opacity:.5;font-size:.48rem">— click multiple to overlay in color</span>
      <button class="gap-filter-btn" onclick="gdClearSelection()" style="margin-left:auto;font-size:.52rem">✕ Clear all</button>
    </div>
  ...
  <div class="live-stats" id="liveStats">
    ...
    <div class="stat-cell"><div class="stat-lbl">f(θ) current</div><div class="stat-val" id="stFTheta">—</div></div>
    ...
    <div class="stat-cell"><div class="stat-lbl">θ total (×π)</div><div class="stat-val" id="stThetaMax">64</div></div>
  </div>
</div>
...
<div class="panels">
  ...
  <div class="panel">
    ...
    <div class="grid-2">
      <div class="cg"><label>A — offset</label><input type="range" id="sA" min="0.1" max="4" step="0.05" value="1.9"><div class="vd" id="dA">1.9</div></div>
      <div class="cg"><label>B — outer freq</label><input type="range" id="sB" min="1" max="12" step="0.5" value="3"><div class="vd" id="dB">3</div></div>
  ...
  <div class="panel">
    ...
    <div class="pb-row">
      ...
      <div class="btn-row">
        ...
        <button class="ctrl gold-ctrl" id="btnExport4K" title="Export 4K PNG of sphere">⬇ 4K</button>
      </div>
    </div>
    <div class="loop-row">
...
<div class="gap-panel panels" style="margin-top:11px">
  <div class="gap-header" id="gapHeader">
    <div class="gap-header-title">⚡ Gap Decomposition &amp; ζ(s) Analysis</div>
    <span class="gap-chevron" id="gapChevron">▼</span>
  </div>
  <div class="gap-body" id="gapBody">
    ...
    <div class="gap-run-row">
      ...
      <button class="ctrl gold-ctrl" onclick="gdClear()" style="font-size:.63rem;padding:6px 11px">✕ Clear</button>
      <button class="ctrl gold-ctrl" id="gdExportPNG" onclick="gdExport4K()" style="display:none">⬇ 4K PNG</button>
      <button class="ctrl gold-ctrl" id="gdExportCSV" onclick="gdExportCSV()" style="display:none">⬇ Full CSV</button>
      <button class="ctrl gold-ctrl" id="gdExportTXT" onclick="gdExportTXT()" style="display:none">⬇ Report TXT</button>
    </div>
    ...
    <!-- Multi-gap color selector — populated after compute -->
    ...
    <div class="gap-status" id="gdStatus">Run decomposition to analyze prime gap contributions to ζ(s). Default: 10M primes.</div>
    ...
    <!-- Gap Wave presets refined by N -->
    <div class="gap-wave-presets" id="gdWavePresets" style="display:none">
      <div class="pg-label" style="color:rgba(255,215,0,.55);margin-bottom:6px">Gap Wave Presets — calibrated for N = <span id="gdPresetN">—</span></div>
      <div class="gap-wave-row" id="gdWaveRow"></div>
    </div>
    ...
    <!-- Convergence to π²/6 dropdown table -->
...
const PRESET_GROUPS = {
  ...
  pgGapWave: [
    { name:'Gap 2 · Twin',       A:1.2, B:1,  C:2,  D:2,   theta:64,  trail:5, gap:2  },
    { name:'Gap 4 · Cousin',     A:1.4, B:2,  C:4,  D:4,   theta:80,  trail:5, gap:4  },
    { name:'Gap 6 · Sexy',       A:1.6, B:3,  C:6,  D:6,   theta:96,  trail:5, gap:6  },
    { name:'Gap 8',              A:1.3, B:4,  C:8,  D:8,   theta:112, trail:4, gap:8  },
    { name:'Gap 10',             A:1.5, B:5,  C:10, D:10,  theta:120, trail:4, gap:10 },
    { name:'Gap 12',             A:1.8, B:6,  C:12, D:12,  theta:128, trail:4, gap:12 },
    { name:'Gap 14',             A:1.1, B:7,  C:14, D:14,  theta:140, trail:3, gap:14 },
    { name:'Gap 18',             A:1.9, B:9,  C:18, D:18,  theta:180, trail:3, gap:18 },
    { name:'Gap 24',             A:2.0, B:12, C:24, D:24,  theta:240, trail:3, gap:24 },
    { name:'Gap 30 · Primorial', A:1.7, B:5,  C:6,  D:30,  theta:300, trail:3, gap:30 },
    { name:'Twin Resonance',     A:1.0, B:1,  C:3,  D:2,   theta:128, trail:6, gap:2  },
    { name:'Cousin Drift',       A:1.2, B:2,  C:5,  D:4,   theta:160, trail:5, gap:4  },
    { name:'Sexy Lattice',       A:1.4, B:3,  C:7,  D:6,   theta:192, trail:5, gap:6  },
    { name:'Primorial Web',      A:1.6, B:5,  C:11, D:30,  theta:240, trail:4, gap:30 },
    { name:'Gap Chorus',         A:1.5, B:6,  C:5,  D:12,  theta:300, trail:4, gap:12 },
    { name:'Superposition',      A:1.0, B:2,  C:7,  D:6,   theta:210, trail:6, gap:6  },
  ],
};
...
// ═══════════════════════════════════════════════════════════════
//  RENDER — sphere with multi-gap color overlay
// ═══════════════════════════════════════════════════════════════
const HUE_BANDS = 24;
...
function renderSphere() {
  ...
  // Pass 1: draw base curve (dimmed if gap selection active)
  if (!hasGapSel) {
    // Normal hue-band rendering
    const bands = [];
    ...
    pCtx.lineWidth=traceThickness;
    pCtx.shadowColor='#ff3db4'; pCtx.shadowBlur=4;
    for(let bi=0; bi<HUE_BANDS; bi++){
      const bd=bands[bi];
      if(!bd.segs.length) continue;
      const hue=295+(bi/HUE_BANDS)*40;
      const avgZ=bd.n>0?bd.bright/bd.n:0;
      const bright=48+avgZ*22;
      pCtx.strokeStyle=`hsl(${hue},100%,${bright}%)`;
      pCtx.globalAlpha=Math.max(0.2, 0.4+avgZ*0.55);
      pCtx.beginPath();
      for(let j=0;j<bd.segs.length;j+=4){
        pCtx.moveTo(bd.segs[j],bd.segs[j+1]);
        pCtx.lineTo(bd.segs[j+2],bd.segs[j+3]);
      }
      pCtx.stroke();
    }
    ...
    // Dim base
    pCtx.lineWidth=traceThickness * 0.6;
    pCtx.shadowBlur=0;
    const bands = [];
    for(let b=0; b<HUE_BANDS; b++) bands.push({segs:[], bright:0, n:0});
    for(let i=startIdx; i<end; i++){
    ...
    for(let bi=0; bi<HUE_BANDS; bi++){
      ...
      if(!bd.segs.length) continue;
      const avgZ=bd.n>0?bd.bright/bd.n:0;
      pCtx.strokeStyle=`rgba(80,100,80,${Math.max(0.05,0.12+avgZ*0.1)})`;
      pCtx.globalAlpha=1;
      pCtx.beginPath();
      for(let j=0;j<bd.segs.length;j+=4){
        pCtx.moveTo(bd.segs[j],bd.segs[j+1]);
    ...
    // Pass 2: per-gap-class colored overlay, each gap collected separately
    // Build per-gap segment buckets
    const gapBuckets = new Map(); // gapVal → {segs:[], col, n, bright}
    selectedGaps.forEach((col, gv) => gapBuckets.set(gv, {segs:[], col, bright:0, n:0}));

    for(let i=startIdx; i<end; i++){
      const p0=precomputed[i-1], p1=precomputed[i];
      if(!p0||!p1) continue;
      const gap0 = p0[4], gap1 = p1[4];
      // Use gap1 (the gap at i), match to any selected gap
      const col0 = gap0 !== null ? selectedGaps.get(gap0) : undefined;
      const col1 = gap1 !== null ? selectedGaps.get(gap1) : undefined;
      const col = col1 || col0;
      if (!col) continue;
      const gv = gap1 !== null && selectedGaps.has(gap1) ? gap1 : gap0;
      const bk = gapBuckets.get(gv);
      if (!bk) continue;
      const [rx1,ry1,rz1]=applyM(rotMat,p0[0],p0[1],p0[2]);
      const [rx2,ry2,rz2]=applyM(rotMat,p1[0],p1[1],p1[2]);
      const [px1,py1]=proj(rx1,ry1,rz1);
      const [px2,py2]=proj(rx2,ry2,rz2);
      const depth=(rz1+rz2)*0.5;
      bk.segs.push(px1,py1,px2,py2);
      bk.bright+=depth; bk.n++;
    }

    gapBuckets.forEach(({segs, col, bright, n})=>{
      if(!segs.length) return;
      const avgZ = n > 0 ? bright/n : 0;
      pCtx.strokeStyle = col;
      pCtx.lineWidth = traceThickness * 1.4;
      pCtx.globalAlpha = Math.max(0.55, 0.7+avgZ*0.28);
      pCtx.shadowColor = col; pCtx.shadowBlur = 7;
      pCtx.beginPath();
      for(let j=0;j<segs.length;j+=4){
        pCtx.moveTo(segs[j],segs[j+1]);
        pCtx.lineTo(segs[j+2],segs[j+3]);
      }
      pCtx.stroke();
    });
    pCtx.globalAlpha=1; pCtx.shadowBlur=0;
...
// ═══════════════════════════════════════════════════════════════
//  RENDER CART — with gap color overlay
// ═══════════════════════════════════════════════════════════════
function renderCart() {
  ...
  if(newEnd > lastCartProg){
    ...
    for(let i=lastCartProg; i<newEnd; i++){
      ...
      if (hasGapSel) {
        if (selCol) {
          strokeCol = selCol; lw = traceThickness * 1.6;
          cbCtx.shadowColor = selCol; cbCtx.shadowBlur = 5;
        } else {
          strokeCol = 'rgba(60,80,60,0.25)'; lw = traceThickness * 0.5;
          cbCtx.shadowBlur = 0;
        }
      } else {
        const hue=295+(i/TOTAL_PTS)*40;
        strokeCol = `hsl(${hue},100%,62%)`;
        lw = traceThickness;
        cbCtx.shadowColor='#ff3db4'; cbCtx.shadowBlur=3;
      }

      cbCtx.strokeStyle = strokeCol;
      cbCtx.lineWidth = lw;
...
document.getElementById('btnAutoRot').addEventListener('click',function(){
  ...
  this.textContent=autoRotate?'⟳ Rotating...':'⟳ Auto-Rotate';
});
document.getElementById('btnResetRot').addEventListener('click',()=>{
  ...
  b.classList.remove('on'); b.textContent='⟳ Auto-Rotate';
});
...
function exportSphere4K() {
  ...
  ctx.fillText(`f(θ) = ${A} + sin(${B}θ)·sin(${C}θ/${D})  |  θ∈[0,${Math.round(thetaTotal/Math.PI)}π]`, 60, SIZE-80);
  ctx.globalAlpha=0.4; ctx.font='32px JetBrains Mono, monospace'; ctx.fillStyle='#ff3db4';
  ctx.fillText('Polar vs Cartesian · Wessen Getachew · @7dview', 60, SIZE-36);
  ctx.globalAlpha=1;
...
function gdClear(){
  ...
  document.getElementById('gdWavePresets').style.display='none';
...
function buildAllGapWavePresets(){
  ...
  if(lbl) lbl.textContent = `Gap Waves · ${maxN.toLocaleString()} primes — all ${gaps.length} families`;
  ...
  gaps.filter(g=>gapProducts[g]).forEach((g, i) => {
    ...
    btn.style.color = col;
    btn.textContent = 'g=' + g;
    btn.title = `Gap ${g}: ${gp.count.toLocaleString()} primes (${(logShare*100).toFixed(2)}% log-ζ)
A=${Ap} B=${Bp} C=${Cp} D=${Dp} θ=${theta}π`;
    btn.onclick = () => {
      ...
      document.getElementById('dTheta').textContent=theta+'π';
...
function buildRefinedWavePresets(){
  if(!gdData) return;
  const { gaps, gapProducts, gapFamilies, maxN, s, totalLog } = gdData;

  // ALL gap families sorted by gap value ascending
  const ranked = gaps.filter(g=>gapProducts[g])
    .map(g=>({ g, logShare:gapProducts[g].logProduct/(totalLog||1), count:gapProducts[g].count }))
    .sort((a,b)=>a.g-b.g);

  const row = document.getElementById('gdWaveRow');
  row.innerHTML = ''; gdActiveWaveBtn = null;

  ranked.forEach(({ g, logShare, count }) => {
    const Bp = Math.max(1, Math.floor(g/4));
    const Cp = Math.min(12, g);
    const Ap = parseFloat((1.0 + logShare * 2.5).toFixed(2));
    const theta = Math.min(300, Math.max(64, g*14));
    const trail = g <= 6 ? 6 : g <= 12 ? 5 : g <= 30 ? 4 : 3;
    const Dp = Math.min(128, g);
    const preset = { name:`Gap ${g}`, A:Ap, B:Bp, C:Cp, D:Dp, theta, trail, gap:g };
    const pct = (logShare*100).toFixed(1);
    const btn = document.createElement('button');
    btn.className = 'gap-wave-btn';
    btn.innerHTML = `g=${g}<span class="n-badge">${pct}%</span>`;
    btn.title = `Gap ${g}: ${count.toLocaleString()} primes, ${pct}% log-ζ\nA=${Ap} B=${Bp} C=${Cp} D=${Dp} θ=${theta}π`;
    btn.onclick = () => {
      if(gdActiveWaveBtn) gdActiveWaveBtn.classList.remove('active');
      btn.classList.add('active'); gdActiveWaveBtn=btn;
      applyPreset(preset, null, true);
    };
    row.appendChild(btn);
  });

  document.getElementById('gdPresetN').textContent = maxN.toLocaleString();
  document.getElementById('gdWavePresets').style.display='block';
}
...
function gdDrawRatio(w, h){
  ...
  const pairs=[
    {lbl:'g₂/g₄\ntwin/cousin', val:c4>0?c2/c4:null, tgt:1.0, col:'#ffd700', note:'HL→1'},
    {lbl:'g₂/g₆\ntwin/sexy',   val:c6>0?c2/c6:null, tgt:0.5, col:'#ff3db4', note:'HL→0.5'},
    {lbl:'g₄/g₆\ncousin/sexy', val:c6>0?c4/c6:null, tgt:0.5, col:'#4ecdc4', note:'HL→0.5'},
  ];
  ...
  gdTitle(ctx, w, `Gap count ratios vs Hardy–Littlewood`);
  gdBuildLegend([]);
}
...
function gdExport4K(){
  ...
  ctx.fillText(`Gap Decomp · ζ(${gdData.s.toFixed(2)}) · N=${gdData.maxN.toLocaleString()}`, 60, 20);
